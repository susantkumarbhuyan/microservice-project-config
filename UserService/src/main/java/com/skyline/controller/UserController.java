...
public class UserController {
	...
	public User ratingHotelFallback(long userId, FeignException e) {
		count++;
		log.info("FallBack is executes because service is down: ", e.toString());
		log.info("Retry Count : {}", count);
		if (e.status() == 403) {
			return User.builder().email("Unauthorized").name(e.getMessage()).userId(23).about("Forbiden").build();
		}
		return User.builder().email("Rating Service is Not Available").name(e.getMessage()).userId(23)
				.about("Wait for some time").build();
	}
